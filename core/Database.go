...
type Database interface {
	...
	NativeQuery(ctx context.Context, model interface{}, query string, args ...interface{}) error
...
//		Debug: true,
//		}
//		db, err := database.NewDatabase(postgres.Open(cfg.DSN), cfg)
//	 -- cho MySQL:
//	 db, err := database.NewDatabase(mysql.Open(cfg.DSN), cfg)
func NewDatabase(dialector gorm.Dialector, cfg *Config) (Database, error) {
...
// NativeQuery retrieves multiple records.
//
// Example:
//
//	var users []User
//	err := db.NativeQuery(ctx, &users, "SELECT * FROM users WHERE age > ?", 18)
//	if err != nil {
//	    log.Fatal(err)
//	}
func (p *Client) NativeQuery(ctx context.Context, model interface{}, query string, args ...interface{}) error {
	return p.WithContext(ctx).Raw(query, args...).Scan(model).Error
}
